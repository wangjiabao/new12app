...
const OperationAppBuySuper = "/api.App/BuySuper"
...
func RegisterAppHTTPServer(s *http.Server, srv AppHTTPServer) {
	...
	r.POST("/api/app_server/buy_super", _App_BuySuper0_HTTP_Handler(srv))
...
func _App_BuySuper0_HTTP_Handler(srv AppHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in BuySuperRequest
		if err := ctx.Bind(&in.SendBody); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationAppBuySuper)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.BuySuper(ctx, req.(*BuySuperRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*BuySuperReply)
		return ctx.Result(200, reply)
	}
}
...
type AppHTTPClient interface {
	...
	BuySuper(ctx context.Context, req *BuySuperRequest, opts ...http.CallOption) (rsp *BuySuperReply, err error)
...
func (c *AppHTTPClientImpl) BuySuper(ctx context.Context, in *BuySuperRequest, opts ...http.CallOption) (*BuySuperReply, error) {
	var out BuySuperReply
	pattern := "/api/app_server/buy_super"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationAppBuySuper))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in.SendBody, &out, opts...)
	if err != nil {
		return nil, err
